...
def get_area_info():
    ...
    # 将列表中的城区对象转换成字典
    for area in area_li:
        area_dict_li.append(Area.to_dict(area))

    # 在这里我们考虑到使用字符串类型来存储，直接存储为json字符串，
    # 然后直接取出来，不需要转换,可以直接把响应信息都放进去
...
def get_user_houses():
    ...
    try:
        # 直接从House中查找户主id(user_id)为登录的id(user_id)
        # House.query.filter_by(user_id=user_id)
        # 或者先查找User中是否存在这个用户，再根据外键查找发布的房源，顺便还可以判断用户是否存在
...
# GET /api/v1.0/houses?sd=2019-08-26&ed=2019-08-28&aid=10&sk=new&p=1
@api.route('/houses')
def get_house_list():
    '''获取房屋的列表信息'''
