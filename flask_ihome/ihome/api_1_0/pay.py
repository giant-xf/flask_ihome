...
def order_pay(order_id):
    ...
    # 判断订单状态
    try:
        order = Order.query.filter(Order.order_id == order_id, Order.user_id == user_id,
...
def save_order_payment_result():
    ...
    if result:
        ...
        try:
            Order.query.filter_by(order_id=order_id).update({"status": "WAIT_COMMENT", "trade_no": trade_no})
            db.session.commit()
        except Exception as e:
            current_app.logger.error(e)
            db.session.rollback()
