...
class User(BaseModel, db.Model):
    ...
    # 加上property装饰器，就会将函数变为属性，属性名即为函数名
    @property
    def password(self):
        '''读取属性的方法'''
        # 调用print(user.password)时被调用
        # 函数的返回值作为属性值，
        # return 'xxxxx'
        raise AttributeError('这个属性只能设置，不能读取')

    # 上面的函数名＋setter，使用这个装饰器，就会将其变成对应的设置属性操作
    @password.setter
    def password(self,value):
        '''
        设置属性的方法user.password='xxxxx',密码明文
        :param value: 设置属性时的值'xxxx',
        :return: 
        '''
...
class House(BaseModel, db.Model):
    ...
    def to_full_dict(self):
        ...
        # 评论信息
        comments = []
        orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE", Order.comment != None).order_by(Order.update_time.desc()).limit(constants.HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS)
        for order in orders:
            comment = {
...
class Order(BaseModel, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    order_id = db.Column(db.String(80), nullable=False)  # 订单编号
